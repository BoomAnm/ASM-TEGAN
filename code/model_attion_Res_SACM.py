...
from attention import EMA as EMA_NET
...
class ACM(nn.Module):
    def __init__(self, channel_num):
        ...
        self.conv_mask = nn.Sequential(nn.Conv2d(128, 100, 3, 1, 1),
                                       BatchNorm(100),
                                       nn.ReLU(),
    ...
    def forward(self, x, seg_imgs):
        ...
        conv_mask = self.conv_mask(out_code)
        conv_mask = torch.sigmoid(conv_mask)

        #1
        out_code_weight = self.conv_weight(out_code)
        out_code_weight = out_code_weight * conv_mask + 1

        out_code_bias = self.conv_bias(out_code)
        out_code_bias = out_code_bias * conv_mask

        #2
        out_code_weight_2 = self.conv_weight(out_code)
        out_code_weight_2 = out_code_weight_2

        out_code_bias_2 = self.conv_bias(out_code)
        out_code_bias_2 = out_code_bias_2

        return (x*out_code_weight_2+out_code_bias_2) + (x * out_code_weight + out_code_bias)
...
class INIT_STAGE_G(nn.Module):
    ...
    def forward(self, z_code, c_code, cnn_code, imgs, mask, word_embs):
        ...
        ema_out_code32 = self.ema(out_code32)

        self.att.applyMask(mask)
        c_code, att = self.att(ema_out_code32, word_embs)
        ...
        h_c_code = torch.cat((ema_out_code32, c_code), 1)
...
class NEXT_STAGE_G(nn.Module):
    ...
    def forward(self, h_code, c_code, word_embs, mask, seg_img):
        ...
        """
        ema_out_code = self.ema(h_code)

        self.att.applyMask(mask)
        c_code, att = self.att(ema_out_code, word_embs)
        ...
        h_c_code = torch.cat((ema_out_code, c_code), 1)
        h_c_c_code = torch.cat((h_c_code, c_code_channel), 1)
